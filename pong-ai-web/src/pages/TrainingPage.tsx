...
const TrainingPage: React.FC = () => {
  ...
  const generateTrainingData = useCallback((size: number): TrainingData => {
    const inputs: number[][] = [];
  ...
  const loadModel = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const extractWeights = useCallback(() => {
    ...
    const layer1Weights = weights[0].arraySync() as number[][];
    const layer1Bias = weights[1].arraySync() as number[];
    const layer2Weights = weights[2].arraySync() as number[][];
    const layer2Bias = weights[3].arraySync() as number[];
